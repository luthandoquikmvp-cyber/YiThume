...
# =================================================
# VALIDATOR LAYER (append-only integration)
# - No existing routes modified.
# - Safe to run in Vercel serverless (tick-based, no background loops).
# =================================================
try:
    from verification_layer import ensure_validator_indexes, register_validator_routes

    try:
        _validator_db = get_db()
        ensure_validator_indexes(_validator_db)
    except Exception as _e:
        print("[validator] indexes skipped:", _e)

    try:
        register_validator_routes(app, get_db)
    except Exception as _e:
        print("[validator] routes not registered:", _e)

except Exception as _e:
    print("[validator] layer not loaded:", _e)
