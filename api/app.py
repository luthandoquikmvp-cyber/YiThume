...
def ensure_indexes(db):
    ...
    db.verification_state.create_index([("updated_at", DESCENDING)])
    db.verification_results.create_index([("transaction_id", ASCENDING), ("checked_at", DESCENDING)])
    db.verification_results.create_index([("rule_id", ASCENDING), ("checked_at", DESCENDING)])
    db.verification_flags.create_index([("status", ASCENDING), ("severity", DESCENDING), ("created_at", DESCENDING)])
    db.verification_flags.create_index([("transaction_id", ASCENDING)], unique=True)
    db.verification_locks.create_index([("expires_at", ASCENDING)], expireAfterSeconds=0)
    db.verification_nodes.create_index([("node_id", ASCENDING)], unique=True)
...
# =================================================
# CONTINUOUS VERIFICATION LAYER (Orders / Deliveries / Inventory)
# -------------------------------------------------
# This module adds:
#   - A resilient background worker (thread-based) with MongoDB distributed locking
#   - Rule-based + "AI-assisted" verification (optional)
#   - Persistent status/results/flags in MongoDB
#   - A real-time UI + JSON status endpoints
#   - A node framework so humans/agents can authenticate and contribute
#
# Notes for deployment:
#   - "Truly continuous" execution requires a long-lived process (VM/container/PaaS worker).
#   - If you deploy to serverless (e.g., Vercel), background threads are not guaranteed to stay alive.
#     In that case, use /v1/verification/tick from an external scheduler (cron) to drive cycles.
# =================================================
...
VERIFICATION_ENABLE = os.environ.get("VERIFICATION_ENABLE", "true").lower() == "true"
VERIFICATION_MODE = os.environ.get("VERIFICATION_MODE", "thread").lower()  # "thread" or "manual"
VERIFICATION_INTERVAL_SEC = int(os.environ.get("VERIFICATION_INTERVAL_SEC", "10"))
VERIFICATION_BATCH_SIZE = int(os.environ.get("VERIFICATION_BATCH_SIZE", "50"))
VERIFICATION_LOCK_TTL_SEC = int(os.environ.get("VERIFICATION_LOCK_TTL_SEC", "45"))
VERIFICATION_REVERIFY_OPEN_FLAGS_EVERY = int(os.environ.get("VERIFICATION_REVERIFY_OPEN_FLAGS_EVERY", "60"))
VERIFICATION_NODE_HEADER = os.environ.get("VERIFICATION_NODE_HEADER", "X-YiThume-Node-Token")
VERIFICATION_LEADER_ID = os.environ.get("VERIFICATION_LEADER_ID", os.environ.get("VERCEL_URL", "local"))
...
def _verif_col(db):
    return {
        "state": db.verification_state,
        "results": db.verification_results,
        "flags": db.verification_flags,
        "locks": db.verification_locks,
        "nodes": db.verification_nodes,
    }


def _acquire_distributed_lock(db, lock_name: str, owner: str, ttl_sec: int) -> bool:
    """
    MongoDB distributed lock with TTL.
    ...
    try:
        res = cols["locks"].find_one_and_update(
            {
                "_id": lock_name,
                "$or": [
                    {"expires_at": {"$lte": now}},
        ...
        # If we got here, we either inserted or updated. Confirm we are the owner.
        doc = cols["locks"].find_one({"_id": lock_name})
        return doc and doc.get("owner") == owner and doc.get("expires_at", now) > now
    except Exception:
        return False


def _touch_state(db, patch: dict):
    cols = _verif_col(db)
    now = _utcnow()
    cols["state"].update_one(
        {"_id": "global"},
...
def _get_state(db) -> dict:
    cols = _verif_col(db)
    doc = cols["state"].find_one({"_id": "global"}) or {}
    return doc
...
class VerificationEngine:
    """
    Verifies transactions (currently: orders/deliveries).
    Stores results & flags in MongoDB so the UI can show real-time status.
    """
    ...
    def start(self):
        if self._thread and self._thread.is_alive():
            return
        self._stop.clear()
        self._thread = threading.Thread(target=self._run_forever, name="verification-worker", daemon=True)
        self._thread.start()
    ...
    def _run_forever(self):
        ...
        while not self._stop.is_set():
            t0 = time.time()
            try:
                self.run_cycle(trigger="thread")
                self._last_cycle_ok = True
                backoff = 1
            except Exception as e:
                self._last_cycle_ok = False
                # record error + backoff (resilient, not a tight dummy loop)
                try:
                    db = get_db()
                    _touch_state(db, {
                        "last_error": str(e),
                        "last_error_at": _utcnow(),
                        "last_cycle_trigger": "thread",
                    })
                except Exception:
                    pass
                backoff = min(backoff * 2, 60)

            # sleep with jitter, but never busy-loop
            elapsed = time.time() - t0
            target = max(1, VERIFICATION_INTERVAL_SEC)
            sleep_for = max(0.2, target - elapsed)
            # jitter up to 15%
            jitter = sleep_for * (0.15 * (secrets.randbelow(1000) / 1000.0))
            time.sleep(sleep_for + jitter + backoff * 0.1)
    ...
    def run_cycle(self, trigger: str = "manual") -> dict:
        ...
        # leader lock (one active verifier across instances)
        got_lock = _acquire_distributed_lock(
            db,
            lock_name="verification_leader",
            owner=str(self.leader_id),
            ttl_sec=VERIFICATION_LOCK_TTL_SEC,
        )
        if not got_lock:
            _touch_state(db, {
                "active": False,
                ...
                "note": "Lock held by another instance; skipping cycle.",
            })
        ...
        if cursor:
            try:
                cursor_dt = cursor if isinstance(cursor, datetime) else datetime.fromisoformat(str(cursor))
            except Exception:
                cursor_dt = now - timedelta(hours=24)
        else:
            cursor_dt = now - timedelta(hours=24)
        ...
        # Pull "transactions" (orders) since cursor
        q = {
            "$or": [
                {"updated_at": {"$gte": cursor_dt}},
                {"created_at": {"$gte": cursor_dt}},
            ]
        }
        orders = list(db.orders.find(q).sort([("updated_at", DESCENDING), ("created_at", DESCENDING)]).limit(VERIFICATION_BATCH_SIZE))

        checked = 0
        flagged = 0
        last_seen_ts = cursor_dt

        for order in orders:
            checked += 1
            ts = order.get("updated_at") or order.get("created_at") or now
            if isinstance(ts, datetime) and ts > last_seen_ts:
                last_seen_ts = ts

            verdict = self.verify_order(db, order)
            # persist results + flags
            tx_id = str(order.get("_id") or order.get("order_id") or order.get("_internal_id") or "")
            self._store_results(db, tx_id, order, verdict)

            if any(not r.ok and r.severity in ("medium", "high") for r in verdict):
                flagged += 1

        # refinement pass: re-verify open flags periodically (even if no new orders)
        last_reverify = state.get("last_reverify_open_at")
        do_reverify = True
        if last_reverify and isinstance(last_reverify, datetime):
            do_reverify = (now - last_reverify).total_seconds() >= VERIFICATION_REVERIFY_OPEN_FLAGS_EVERY

        reverified = 0
        if do_reverify:
            open_flags = list(db.verification_flags.find({"status": "open"}).sort([("severity", DESCENDING), ("created_at", DESCENDING)]).limit(20))
            for f in open_flags:
                order = db.orders.find_one({"_id": ObjectId(f.get("order_object_id"))}) if f.get("order_object_id") else None
                if order:
                    verdict = self.verify_order(db, order, refinement=True)
                    tx_id = str(order.get("_id") or order.get("order_id") or order.get("_internal_id") or "")
                    self._store_results(db, tx_id, order, verdict, refinement=True)
                    reverified += 1
            _touch_state(db, {"last_reverify_open_at": now})
        ...
        _touch_state(db, {
            ...
            "last_cycle_summary": {
                "checked": checked,
                "flagged": flagged,
                "reverified_open": reverified,
                "cursor_from": cursor_dt,
                "cursor_to": last_seen_ts,
            }
        })

        return {"ok": True, "checked": checked, "flagged": flagged, "reverified_open": reverified}

    # --------------------------
    # Verification logic
    # --------------------------
    def verify_order(self, db, order: dict, refinement: bool = False) -> list:
        """
        Deterministic business-rule verification.
        "AI-assisted" scoring/summary can be layered in later (or via nodes).
        """
        results = []
        ...
        missing = []
        for k in ["order_id", "status", "customer"]:
            if not order.get(k):
                missing.append(k)
        if missing:
            results.append(RuleResult(
                rule_id="order.required_fields",
                ok=False,
                severity="high",
                message=f"Missing required fields: {', '.join(missing)}",
                details={"missing": missing, "order_object_id": oid},
            ))
        else:
            results.append(RuleResult(
                rule_id="order.required_fields",
        ...
        # Rule 2: status -> delivered_at consistency
        if status in ("delivered", "complete", "completed"):
            if not delivered_at:
                results.append(RuleResult(
                    rule_id="order.delivered_requires_timestamp",
                    ok=False,
                    severity="high",
                    message="Order marked delivered/completed but delivered_at is missing",
                ...
                results.append(RuleResult(
                    ...
                    ok=True,
                    severity="low",
                    message="Delivered timestamp present for delivered order",
                    details={},
                ))
        else:
            results.append(RuleResult(
                ...
                ok=True,
                severity="low",
                message="Not delivered; delivered_at not required",
                details={"status": status},
            ))

        # Rule 3: totals sanity (if fields exist)
        total = order.get("total") or order.get("total_amount") or order.get("amount_total")
        ...
        if isinstance(items, list) and items:
            try:
                ...
                for it in items:
                    ...
                    price = float(it.get("price") or it.get("unit_price") or it.get("amount") or 0)
                    computed += qty * price
        ...
        if total is not None and computed is not None:
            try:
                ...
                delta = abs(total_f - computed)
                if delta > 0.5:  # tolerance
                    results.append(RuleResult(
                        rule_id="order.total_matches_items",
                        ok=False,
                        severity="medium",
                        message="Order total does not match sum(items)",
                        details={"total": total_f, "computed": computed, "delta": delta},
                    ))
                else:
                    results.append(RuleResult(
                        rule_id="order.total_matches_items",
                        ok=True,
                        severity="low",
                        message="Order total matches items (within tolerance)",
                        details={"total": total_f, "computed": computed, "delta": delta},
                    ))
            except Exception:
                results.append(RuleResult(
                    ...
                    ok=False,
                    severity="low",
                    message="Could not parse totals for verification",
                    details={},
                ))
        else:
            results.append(RuleResult(
                ...
                ok=True,
                severity="low",
                message="Total or items missing; skipped total check",
                details={"has_total": total is not None, "has_items": bool(items)},
            ))

        # Rule 4: assigned driver exists (if assigned)
        drv = order.get("assigned_driver_id") or order.get("driver_id")
        if drv:
            driver = None
            try:
                driver = db.drivers.find_one({"$or": [{"_internal_id": drv}, {"driver_id": drv}, {"_id": ObjectId(drv)}]})
            except Exception:
                driver = db.drivers.find_one({"$or": [{"_internal_id": drv}, {"driver_id": drv}]})
            if not driver:
                results.append(RuleResult(
                    rule_id="order.assigned_driver_exists",
                    ok=False,
                    severity="medium",
                    message="Order has assigned driver but driver record not found",
                    details={"assigned_driver_id": drv},
                ))
            else:
                results.append(RuleResult(
                    rule_id="order.assigned_driver_exists",
                    ok=True,
                    severity="low",
                    message="Assigned driver exists",
                    details={"assigned_driver_id": drv},
                ))
        else:
            results.append(RuleResult(
                ...
                ok=True,
                severity="low",
                message="No driver assigned; skipped driver existence check",
                details={},
            ))

        # Rule 5: delivery fee non-negative (if present)
        ...
        if fee is not None:
            try:
                ...
                if fee_f < 0:
                    results.append(RuleResult(
                        rule_id="order.delivery_fee_non_negative",
                        ok=False,
                        severity="high",
                        message="Delivery fee is negative",
                        details={"delivery_fee": fee_f},
                    ))
                else:
                    results.append(RuleResult(
                        rule_id="order.delivery_fee_non_negative",
                        ok=True,
                        severity="low",
                        message="Delivery fee is non-negative",
                        details={"delivery_fee": fee_f},
                    ))
            except Exception:
                results.append(RuleResult(
                    ...
                    ok=False,
                    severity="low",
                    message="Could not parse delivery_fee",
            ...
            results.append(RuleResult(
                ...
                ok=True,
                severity="low",
                message="No delivery_fee field; skipped fee check",
                details={},
            ))

        # Optional: lightweight "AI" anomaly score (heuristic) — safe default
        score = 0.0
        for r in results:
            if not r.ok:
                score += {"low": 0.2, "medium": 0.6, "high": 1.0}.get(r.severity, 0.4)
        score = min(1.0, round(score / 3.0, 3))
        results.append(RuleResult(
            rule_id="ai.anomaly_score",
            ok=score < 0.5,
            severity="medium" if score >= 0.5 else "low",
            message=f"Anomaly score={score}",
            details={"score": score, "refinement": refinement},
        ))
    ...
    # Persistence
    # --------------------------
    def _store_results(self, db, tx_id: str, order: dict, results: list, refinement: bool = False):
        cols = _verif_col(db)
        now = _utcnow()

        # Store each rule result (append-only, but bounded by TTL later if desired)
        for r in results:
            doc = {
                "transaction_id": tx_id,
                "order_object_id": str(order.get("_id")),
            ...
            cols["results"].insert_one(doc)

        # Compute overall flag status
        worst = None
        for r in results:
            if not r.ok:
                if worst is None:
                    ...
                    order_rank = {"low": 1, "medium": 2, "high": 3}
                    if order_rank.get(r.severity, 1) > order_rank.get(worst.severity, 1):
                        worst = r

        if worst:
            # upsert open flag
            cols["flags"].update_one(
                {"transaction_id": tx_id},
                {"$set": {
                    "transaction_id": tx_id,
                    "order_object_id": str(order.get("_id")),
            ...
            cols["flags"].update_one(
                {"transaction_id": tx_id, "status": "open"},
                {"$set": {"status": "closed", "closed_at": now, "updated_at": now}},
            )


# Global singleton engine (safe for Flask import style)
_verification_engine = VerificationEngine(leader_id=VERIFICATION_LEADER_ID)


def _maybe_start_verification_worker():
    """
    Starts the background verifier if enabled.
    On serverless platforms this may not keep running; use /v1/verification/tick as a fallback.
    """
    if not VERIFICATION_ENABLE:
        return
    if VERIFICATION_MODE != "thread":
        return
    try:
        _verification_engine.start()
    except Exception:
        pass


# Start worker on import (best-effort; resilient to environments where threads are blocked)
_maybe_start_verification_worker()


# -------------------------------------------------
# VERIFICATION: Node auth helpers
# -------------------------------------------------
def _hash_token(token: str) -> str:
    return hashlib.sha256(token.encode("utf-8")).hexdigest()


def _require_admin_secret(req) -> bool:
    # Reuse existing ADMIN_SECRET patterns (query/body)
    pin = req.args.get("admin_secret")
    if not pin and ALLOW_PIN_PARAM:
        try:
            pin = (req.json or {}).get("admin_secret")
        except Exception:
            pin = None
    if not pin:
        pin = req.headers.get("X-Admin-Secret")
    return bool(pin) and hmac.compare_digest(str(pin), str(ADMIN_SECRET))


def _get_node_from_request(db, req):
    token = req.headers.get(VERIFICATION_NODE_HEADER) or ""
    if not token:
        return None
    h = _hash_token(token)
    node = db.verification_nodes.find_one({"token_hash": h, "active": True})
    return node


# -------------------------------------------------
# VERIFICATION ROUTES (JSON + UI)
# -------------------------------------------------
@app.get("/v1/verification/status")
def v1_verification_status():
    db = get_db()
    state = _get_state(db)
    open_flags = list(db.verification_flags.find({"status": "open"}).sort([("severity", DESCENDING), ("created_at", DESCENDING)]).limit(25))
    recent = list(db.verification_results.find({}).sort([("checked_at", DESCENDING)]).limit(50))

    # tighten payload sizes
    for f in open_flags:
        f["_id"] = str(f.get("_id"))
    for r in recent:
        r["_id"] = str(r.get("_id"))
    ...
    return jsonify({
        "ok": True,
        "engine": {
            "enabled": VERIFICATION_ENABLE,
            "mode": VERIFICATION_MODE,
            "running_thread": _verification_engine.is_running(),
            "leader_id": VERIFICATION_LEADER_ID,
...
@app.post("/v1/verification/tick")
def v1_verification_tick():
    """
    Manual/scheduled trigger for environments without persistent workers (serverless).
    Protectable via ADMIN_SECRET if desired.
    """
    if os.environ.get("VERIFICATION_TICK_REQUIRE_ADMIN", "false").lower() == "true":
        if not _require_admin_secret(request):
            return jsonify({"ok": False, "error": "unauthorized"}), 401

    res = _verification_engine.run_cycle(trigger="http_tick")
    return jsonify({"ok": True, "result": res})


@app.post("/v1/verification/nodes/register")
def v1_verification_register_node():
    """
    Admin-only: creates a node credential so humans/agents can contribute.
    Returns a one-time token (only shown once).
    """
    if not _require_admin_secret(request):
        return jsonify({"ok": False, "error": "unauthorized"}), 401
    ...
    node_name = (payload.get("name") or "node").strip()[:64]
    node_type = (payload.get("type") or "ai").strip()[:32]

    node_id = "node_" + uuid.uuid4().hex[:12]
    token = secrets.token_urlsafe(32)
    token_hash = _hash_token(token)

    db.verification_nodes.insert_one({
        "node_id": node_id,
        "name": node_name,
        "type": node_type,
    ...
    return jsonify({
        ...
        "header": VERIFICATION_NODE_HEADER,
    })


@app.post("/v1/verification/nodes/heartbeat")
def v1_verification_node_heartbeat():
    db = get_db()
    node = _get_node_from_request(db, request)
    if not node:
        return jsonify({"ok": False, "error": "unauthorized"}), 401

    db.verification_nodes.update_one(
        {"node_id": node["node_id"]},
        {"$set": {"last_seen_at": _utcnow(), "last_ip": request.remote_addr}},
    )
    return jsonify({"ok": True, "node_id": node["node_id"]})


@app.post("/v1/verification/nodes/report")
def v1_verification_node_report():
    """
    Nodes can submit findings for a transaction (order/delivery/etc).
    This enables human review or AI agents to add supplemental checks.
    """
    db = get_db()
    node = _get_node_from_request(db, request)
    if not node:
    ...
    finding = {
        "transaction_id": tx_id,
        "node_id": node["node_id"],
        "rule_id": str(payload.get("rule_id") or "node.finding"),
    ...
    db.verification_results.insert_one(finding)

    # If node flags it as not ok -> open/update a flag record
    if not finding["ok"]:
        db.verification_flags.update_one(
            {"transaction_id": tx_id},
            {"$set": {
                "transaction_id": tx_id,
                "status": "open",
                "severity": finding["severity"],
                "summary": finding["message"] or f"Flagged by {node['node_id']}",
                "last_rule_id": finding["rule_id"],
                "updated_at": _utcnow(),
...
def verification_ui():
    """
    Lightweight UI page (no templates needed) that polls /v1/verification/status.
    """
    html = f"""<!doctype html>
<html>
<head>
  <meta charset="utf-8"/>
  <meta name="viewport" content="width=device-width, initial-scale=1"/>
  <title>YiThume — Verification</title>
  <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-slate-950 text-slate-100">
  <div class="max-w-6xl mx-auto p-4">
    <div class="flex items-center justify-between gap-3">
      <div>
        <h1 class="text-2xl font-bold">Verification Layer</h1>
        <p class="text-slate-400 text-sm">Real-time health, recent checks, and flagged issues.</p>
      </div>
      <div class="flex items-center gap-2">
        <a href="/dashboard.html" class="px-3 py-2 rounded bg-slate-800 hover:bg-slate-700 text-sm">Dashboard</a>
        <button id="tickBtn" class="px-3 py-2 rounded bg-emerald-600 hover:bg-emerald-500 text-sm">Run tick</button>
      </div>
    </div>

    <div class="grid md:grid-cols-3 gap-3 mt-4">
      <div class="rounded-xl bg-slate-900 p-4">
        <div class="text-slate-400 text-xs">Engine</div>
        <div id="engine" class="mt-1 font-mono text-sm"></div>
      </div>
      <div class="rounded-xl bg-slate-900 p-4">
        <div class="text-slate-400 text-xs">Last cycle</div>
        <div id="cycle" class="mt-1 font-mono text-sm"></div>
      </div>
      <div class="rounded-xl bg-slate-900 p-4">
        <div class="text-slate-400 text-xs">Open flags</div>
        <div id="openCount" class="mt-1 font-mono text-2xl"></div>
      </div>
    </div>
    ...
    <div class="grid md:grid-cols-2 gap-3 mt-4">
      <div class="rounded-xl bg-slate-900 p-4">
        <div class="flex items-center justify-between">
          <h2 class="font-semibold">Flagged issues</h2>
      ...
      <div class="rounded-xl bg-slate-900 p-4">
        ...
        <div id="results" class="mt-3 space-y-2 max-h-[70vh] overflow-auto pr-2"></div>
      </div>
    </div>
  </div>
...
async function fetchStatus() {{
  const res = await fetch('/v1/verification/status', {{ cache: 'no-store' }});
  const j = await res.json();

  el('engine').textContent = JSON.stringify(j.engine, null, 2);
  el('cycle').textContent  = JSON.stringify(j.state?.last_cycle_summary || {{}}, null, 2);
  el('openCount').textContent = (j.open_flags || []).length;

  const flags = (j.open_flags || []).map(f => {{
    return `<div class="rounded-lg border border-slate-800 p-3">
      <div class="flex items-center justify-between">
        <div class="font-mono text-xs text-slate-400">${{f.transaction_id}}</div>
        <div class="text-xs px-2 py-0.5 rounded bg-slate-800">${{f.severity}}</div>
      </div>
      <div class="mt-1">${{(f.summary || '').replace(/</g,'&lt;')}}</div>
      <div class="mt-1 text-xs text-slate-500">updated: ${{(f.updated_at || '').toString()}}</div>
    </div>`;
  }}).join('');
  el('flags').innerHTML = flags || '<div class="text-slate-500 text-sm">No open issues 🎉</div>';

  const results = (j.recent_results || []).slice(0,50).map(r => {{
    return `<div class="rounded-lg border border-slate-800 p-3">
      <div class="flex items-center justify-between">
        <div class="font-mono text-xs text-slate-400">${{r.transaction_id}} • ${{r.rule_id}}</div>
        <div class="text-xs px-2 py-0.5 rounded ${{r.ok ? 'bg-emerald-700' : 'bg-rose-700'}}">${{r.ok ? 'OK' : 'FAIL'}}</div>
      </div>
      <div class="mt-1 text-sm">${{(r.message || '').replace(/</g,'&lt;')}}</div>
      <div class="mt-1 text-xs text-slate-500">${{(r.checked_at || '').toString()}}</div>
    </div>`;
  }}).join('');
  el('results').innerHTML = results || '<div class="text-slate-500 text-sm">No results yet.</div>';

  return j;
}}

async function tick() {{
  await fetch('/v1/verification/tick', {{
    method: 'POST',
    headers: {{ 'Content-Type': 'application/json' }},
    body: JSON.stringify({{}})
  }});
